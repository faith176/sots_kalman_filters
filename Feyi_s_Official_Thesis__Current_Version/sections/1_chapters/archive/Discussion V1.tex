...
% \subsection{Model-Driven Analysis through Model-to-Model Transformations}

% A key opportunity for extending the proposed approach lies in the use of model-driven engineering (MDE) techniques, particularly model-to-model transformations, to enable stronger analysis of statechart-driven behaviour. In MDE, models serve as primary artifacts across different stages of development, and transformations provide a mechanism for deriving additional representations from existing models \cite{biehl2010literature}. These representations allow behaviour to be defined once while supporting multiple analytical views \cite{bezivin2006model}.

% Statecharts provide a structured and executable representation of lifecycle-driven behaviour, making them well suited for defining and enforcing system behaviour at runtime. However, they do not explicitly represent properties such as resource consumption, synchronization structure, or global system conditions. These aspects are therefore difficult to analyse directly within the formalism. Petri nets provide a foundational model for capturing concurrency, synchronization, and resource flow \cite{peterson1981petri}. Through model-to-model transformations, statecharts can be converted into equivalent Petri net representations, enabling analysis using techniques such as temporal logic queries \cite{cortadella1995synthesizing, hei2011automatic}. This supports the identification of deadlocks, unreachable states, and resource contention scenarios that are difficult to observe directly in the original model. These insights can then inform refinements to the statechart, such as introducing additional constraints or guarded transitions to enforce safe behaviour. More broadly, this approach supports reuse of behavioural models while maintaining consistency across representations, strengthening the integration between design, execution, and verification in SoTS.