...
An \emph{Event} represents an observation produced by an event source.  Events are propagated using an \emph{Event Stream}, which provides a sequence of events to downstream elements of the system. \emph{Event sources} generate observations at specifc intervals but are assumed to be unreliable, allowing events to be absent.
...
The event processor evaluates atomic and complex event patterns over the event stream. It consumes both observed and reconstructed events and applies pattern definitions to detect higher-level events. Confidence associated with events may be incorporated into pattern evaluation. This allows pattern detection to account for uncertainty introduced by reconstructed observations.
