...
The literature review (\secref{sec:literature-review-key-takeaways}) identifies a reliability gap in SoTS: while system behaviour depends on continuous streams of events, existing approaches provide limited support for interpreting system behaviour when expected observations do not arrive. This gap is particularly evident in how event streams are processed during execution. Event-driven processing derives higher-level system behaviour by detecting patterns over incoming events. When observations are missing, event-based reasoning may diverge from the system state it is intended to represent. This chapter introduces an abstract model of event-driven operation that formalizes event generation, timing, and processing in the presence of missing observations. The model provides the foundation for the for the system architecture presented in \chref{ch:architecture}.
...
The model assumes that missing observations arise from finite, short-term interruptions rather than prolonged outages. This assumption constrains the applicability of the model to scenarios in which prediction-based inference is appropriate over limited time periods. Specifically, the prediction models used within the system are assumed to be capable of handling the expected average duration of such interruptions and to provide uncertainty information over the corresponding prediction interval.
...
\end{itemize}
\noindent
% Classification reflects the information available at the time of evaluation.
